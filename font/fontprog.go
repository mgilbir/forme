...
// ParseType1 parses a Type 1 font program — a PDF's /FontFile, in either the
// binary or the hexadecimal eexec form: the eexec-encrypted private portion
// holds the CharStrings dictionary, which gives the glyph names and the
// hsbw/sbw widths, and the cleartext portion the FontMatrix. It returns nil for
// bytes that are not a Type 1 program.
//
// This engine never loads a Type 1 face; shape reads sfnt and CFF. The reader
// is here for callers that validate PDFs — which glyphs an embedded Type 1
// program defines, whether it has .notdef, its widths and CharSet — and it is
// exported API for them: it was once removed as unreachable because nothing in
// this repository called it, and a caller outside it did (issue #741). It is
// held to the same bounds, budget and fuzzing as the sfnt and CFF readers, so
// that no other module needs a Type 1 parser of its own.
...
func extractType1FontMatrix(data []byte) float64 {
	...
	// The array's tokens are separated by PostScript's white space, the six
	// characters isWhitespace names, and not by Unicode's: a Type 1 program is
	// bytes, and a no-break space inside one is part of a token.
	fields := strings.FieldsFunc(s[lb+1:lb+rb], func(r rune) bool {
		return r < 0x80 && isWhitespace(byte(r))
	})
	if len(fields) < 1 {
		return 0
	}
