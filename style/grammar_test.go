...
func TestTheValueGrammarAgreesWithTheSpecifications(t *testing.T) {
	...
	validCases := []c{
		...
		{"grid-row", "1 / span 2"}, {"grid-column-start", "span name"},
		{"grid-area", "a / b / c / d"}, {"grid-row-start", "-1"},
	...
	invalidCases := []c{
		...
		{"grid-template-columns", "1fr foo(1)"}, {"grid-row", "span -1"},
		{"grid-row", "0"}, {"grid-area", "a / b / c / d / e"},
