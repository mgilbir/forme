...
func TestALineEndDiscountsOnlyTheSpacingTheRunActuallyHas(t *testing.T) {
	...
	if NewBreaker(nil).overflows(0, latin, width, Rightmost{}) {
	...
	if !NewBreaker(nil).overflows(0, arabic, width, Rightmost{}) {
	...
	if NewBreaker(nil).overflows(0, atomic, width, Rightmost{}) {
