	...
	for _, text := range balanceTexts {
		...
		for _, width := range []float64{30, 60, 100, 200, 400} {
			for _, indent := range []float64{0, 24} {
				plain, _ := br.countLines(items, nil, u(width), u(indent), 99)
				banded, _ := br.countLinesInBands(items, nil, []style.Unit{u(width)}, u(width), u(indent), 99)
				if plain != banded {
...
func TestBandedBalancingAgreesWithPlainBalancingWhenTheBandsAreUniform(t *testing.T) {
	...
	for _, text := range balanceTexts {
		...
		for _, width := range []float64{60, 100, 200, 400} {
			plain := br.BalanceWidth(items, nil, u(width), 0)
			banded := br.BalanceWidthInBands(items, nil, []style.Unit{u(width)}, u(width), 0)
			if plain != banded {
...
func TestABalancedWidthNeverWidensTheBoxAndNeverAddsALine(t *testing.T) {
	...
	for _, text := range balanceTexts {
		...
		for _, bs := range bandSets {
			for _, width := range []float64{60, 100, 200, 400} {
				cap := br.BalanceWidthInBands(items, nil, bs.bands, u(width), 0)
				...
				full, _ := br.countLinesInBands(items, nil, bs.bands, u(width), 0, MaxBalanceLines+1)
				capped, _ := br.countLinesInBands(items, nil, bs.bands, cap, 0, MaxBalanceLines+2)
				if capped > full {
	...
	for _, text := range balanceTexts {
		...
		for _, bs := range bandSets {
			for _, width := range []float64{100, 200, 400} {
				...
				full, _ := br.countLinesInBands(items, nil, bs.bands, u(width), 0, MaxBalanceLines+1)
				narrower, _ := br.countLinesInBands(items, nil, bs.bands, cap.Sub(1), 0, MaxBalanceLines+2)
				if narrower <= full {
...
func TestANarrowerBandNeverNeedsFewerLines(t *testing.T) {
	...
	for _, text := range balanceTexts {
		...
		for _, width := range []float64{100, 200, 400} {
			...
			nWide, _ := br.countLinesInBands(items, nil, wide, u(width), 0, 99)
			nNarrow, _ := br.countLinesInBands(items, nil, narrow, u(width), 0, 99)
			if nNarrow < nWide {
	...
	for _, text := range balanceTexts {
		...
		for _, band := range []float64{30, 60, 100} {
			for _, width := range []float64{200, 400} {
				...
				banded, _ := br.countLinesInBands(items, nil, []style.Unit{u(band)}, u(width), 0, 99)
				plain, _ := br.countLines(items, nil, u(band), 0, 99)
				if banded != plain {
...
func TestBandsRunningOutRepeatTheLastOne(t *testing.T) {
	...
	for _, text := range balanceTexts {
		...
		for _, width := range []float64{100, 200, 400} {
			a, _ := br.countLinesInBands(items, nil, short, u(width), 0, 99)
			b, _ := br.countLinesInBands(items, nil, long, u(width), 0, 99)
			if a != b {
...
func TestTheNthLineGetsTheNthBand(t *testing.T) {
	...
	for _, text := range balanceTexts {
		...
		more, _ := br.countLines(items[next:], nil, u(200), 0, 99)
		want += more

		got, _ := br.countLinesInBands(items, nil, bands, u(400), 0, 99)
		if got != want {
...
func TestTheIndentIsTakenOffTheFirstLineInBandsToo(t *testing.T) {
	...
	plainNoIndent, _ := br.countLines(items, nil, u(width), 0, 99)
	plainIndented, _ := br.countLines(items, nil, u(width), u(indent), 99)
