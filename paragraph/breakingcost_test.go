...
// struct, and a paragraph of sixteen thousand of them read a linear walk as a
// factor of seven, which is a cache and not a curve.
//
...
// plant produced, measured through costtest.Time, is written beside each.

// checkLinear fails when four times the input took more than eight times as
// long.
func checkLinear(t *testing.T, what string, n int, run func(n int)) {
	t.Helper()
	c := costtest.Time(t, what, func() { run(n) }, func() { run(4 * n) })
	if c.Ratio > 8 {
		t.Errorf("%s: %d took %v and %d took %v, a factor of %.1f for four times the "+
			"input; linear work is four and quadratic sixteen",
			what, n, c.Small, 4*n, c.Large, c.Ratio)
	}
}
...
func TestResumingAWordCostsTheLineNotThePosition(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		// Planted (counted again): a factor of 12.6.
		name: "the characters in front of the cut", n: 16000,
		ask: func() func(n int) {
		...
		// Planted (the map asked every time): a factor of 13.7.
		name: "where the rest may be cut", n: 32000,
		ask: func() func(n int) {
		...
		// 13.0.
		name: "the shaping a head is measured from", n: 32000,
		ask: func() func(n int) {
			words := word("a", TextSpacing{}, false)
			return func(n int) {
				w := words(n)
		...
		// no table at all. Planted: the letters counted, a factor of 15.9;
		// the word separators counted, 15.9.
		name: "the spacing a stretch carries", n: 4000,
		ask: func() func(n int) {
		...
		// Planted (counted): a factor of 15.9.
		name: "whether a cursive stretch is tracked", n: 4000,
		ask: func() func(n int) {
		...
		// text): a factor of 15.8.
		name: "the advance of an upright stretch", n: 4000,
		ask: func() func(n int) {
		...
		t.Run(tc.name, func(t *testing.T) {
			checkLinear(t, tc.name, tc.n, tc.ask)
		})
	}
}
...
func TestALineDoesNotWalkTheRestOfTheParagraph(t *testing.T) {
	...
	// of filling each one.
	checkLinear(t, "breaking a paragraph of words", 1000, func(n int) {
...
// again): a factor of 15.9 for the clamp and 14.3 for the count.
func TestBalancingProbesDoNotWalkTheRestOfTheParagraph(t *testing.T) {
	...
	checkLinear(t, "balancing a clamped paragraph", 500, func(n int) {
		br.BalanceClampedWidth(paragraph(n), nil, u(300), 0, u(12), maxClampLines)
	})
	checkLinear(t, "counting the lines of a paragraph", 1000, func(n int) {
...
// Planted, building the text again is a factor of 10.3 and the memo entry a
// factor of 11.7.
func TestTheRunsOfAMergeGroupDoNotEachBuildTheGroup(t *testing.T) {
	...
	checkLinear(t, "measuring every run of one merge group", 16000, func(n int) {
		br := NewBreaker(nil)
...
// of the three letters it is about. Planted: a factor of 15.9.
func TestLithuanianLowercaseAsksAboutTheLetterFirst(t *testing.T) {
	text := cached(func(n int) string { return "I" + strings.Repeat("\u0316", n) })
	checkLinear(t, "lowercasing a run of marks in Lithuanian", 4000, func(n int) {
