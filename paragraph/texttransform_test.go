...
func TestACharacterWhoseCaseIsNotOneCharacter(t *testing.T) {
	for _, tc := range []struct {
		...
		got, _ := TransformText(tc.in, tc.kind, WordClosed, "")
		if got != tc.want {
...
func TestTextWithNoFullMappingIsUnchangedByTheTable(t *testing.T) {
	for _, in := range []string{
		...
		for _, tc := range []struct {
			...
			if got, _ := TransformText(in, tc.kind, WordClosed, ""); got != tc.want {
...
func TestTheTransformsAreAppliedInTheSpecifiedOrder(t *testing.T) {
	for _, tc := range []struct {
		...
		got, _ := TransformText(tc.in, TransformOf(tc.value), WordClosed, "")
		if got != tc.want {
...
func TestALetterNumberIsALetterToCapitalize(t *testing.T) {
	for _, tc := range []struct{ text, want, what string }{
		...
		got, _ := TransformText(tc.text, TransformCapitalize, WordClosed, "")
		if got != tc.want {
