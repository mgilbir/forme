...
// spanCodes is the shaped glyphs as the two-byte codes a composite font gives
// them, which is what this asserted on when it went through the span API.
func spanCodes(t *testing.T, f *Face, s string) []byte {
	t.Helper()
	glyphs, missing := f.ShapeGlyphs(s)
	if missing != 0 {
		t.Fatalf("ShapeGlyphs(%q): %d runes have no glyph", s, missing)
	}
	return codesOfGlyphs(glyphs)
}

func codesOfGlyphs(glyphs []Glyph) []byte {
	var out []byte
	for _, g := range glyphs {
		out = append(out, byte(g.GID>>8), byte(g.GID))
	}
	return out
}
...
func withCodes(t *testing.T, f *Face, s string) []byte {
	t.Helper()
	glyphs, missing := f.ShapeGlyphsWith(s, "salt")
	if missing != 0 {
		t.Fatalf("ShapeGlyphsWith(%q): %d runes have no glyph", s, missing)
	}
	return codesOfGlyphs(glyphs)
}
