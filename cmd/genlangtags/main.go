...
//
//	make language-tags
//	go run ./cmd/genlangtags -source <url> -sha256 <hex> -in testdata/harfbuzz-langtags/hb-ot-tag-table.hh \
//		-license-source <url> -license-sha256 <hex> -license testdata/harfbuzz-langtags/COPYING -out shape/langtags.go
//
...
//
// HarfBuzz is under the "Old MIT" licence, which permits all of this provided
// "the above copyright notice and the following two paragraphs appear in all
// copies". The table is a copy of part of HarfBuzz, so its COPYING goes into
// the table's header whole, as the file states it: taken at the same release
// as the header, pinned by its own SHA-256, which the Makefile's fetch checks
// too. It used to say "see its COPYING" and carry none of it, which is the
// one thing the licence asks.
package main
...
func main() {
	...
	licensePath := flag.String("license", "testdata/harfbuzz-langtags/COPYING", "HarfBuzz's COPYING, at the same release")
	licenseSource := flag.String("license-source", "", "the URL COPYING was fetched from")
	licensePin := flag.String("license-sha256", "", "the SHA-256 COPYING is pinned to")
	flag.Parse()
	if *source == "" || *pin == "" {
		log.Fatal("no -source or -sha256: the table has to say which header it is from")
	}
	if *licenseSource == "" || *licensePin == "" {
		log.Fatal("no -license-source or -license-sha256: the table has to carry HarfBuzz's " +
			"notice, and say which copy of it")
	}
	license, err := os.ReadFile(*licensePath)
	if err != nil {
		log.Fatalf("reading %s: %v (run `make language-tags`)", *licensePath, err)
	}
	if sum := sha256.Sum256(license); hex.EncodeToString(sum[:]) != *licensePin {
		log.Fatalf("%s has SHA-256 %x, and the notice is pinned to %s", *licensePath, sum, *licensePin)
	}
	if !strings.Contains(string(license), "Permission is hereby granted") {
		log.Fatalf("%s grants no permission this can see; it is not HarfBuzz's COPYING", *licensePath)
	}
	...
	fmt.Fprintf(&b, "//\n// Source: %s\n// SHA-256: %s\n//\n", *source, *pin)
	fmt.Fprintf(&b, "// HarfBuzz's notice, which its licence requires in every copy, as its\n")
	fmt.Fprintf(&b, "// COPYING states it.\n//\n// Source: %s\n// SHA-256: %s\n//\n", *licenseSource, *licensePin)
	for _, line := range strings.Split(strings.TrimRight(string(license), "\n"), "\n") {
