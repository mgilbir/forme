...
// rather than as data — and shape/normalize.go computes them. What this checks
...
// The Hangul blocks shape/normalize.go computes over. They are checked rather
	...
	for _, ccc := range reorderedClasses {
		if !present[ccc] {
			fmt.Fprintf(os.Stderr, "gencanonical: no character has combining class %d;\n"+
				"shape/normalize.go reorders it, so either the class was retired upstream\n"+
...
// checkHangul verifies the blocks shape/normalize.go computes Hangul over.
//
...
func checkHangul(chars map[rune]charData) {
	fail := func(format string, args ...any) {
		fmt.Fprintf(os.Stderr, "gencanonical: "+format+"\n"+
			"shape/normalize.go computes Hangul from these blocks, so it would now\n"+
