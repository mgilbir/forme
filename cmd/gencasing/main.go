...
// more than one. And its full case folding, from CaseFolding.txt.
//
...
// # Case folding
//
// Folding is not lowercasing. It is the mapping Unicode §3.13 defines for
// comparing strings without regard to case — toCasefold, and default caseless
// matching (D144) is two strings whose foldings are equal — and it differs from
// the lowercase exactly where comparison needs it to: "ß" folds to "ss", so
// that it meets "SS"; final "ς" folds to "σ", as "Σ" does; "ﬁ" folds to "fi".
// CSS Fonts 4 §5.1 matches font family names by it, and names the statuses:
// "the case mappings with status field "C" or "F" in the CaseFolding.txt
// file". C is the folding every character with one has in common, one
// character to one; F is the full folding where it is more than one. S, the
// simple folding for a character whose full one is longer, is the other
// choice and not this one, and T is the Turkic tailoring of I and İ, which §5.1
// rules out ("without applying any language-specific tailorings").
//
//	go run ./cmd/gencasing -version <X.Y.Z> <UnicodeData.txt> <SpecialCasing.txt> <CaseFolding.txt> > paragraph/casingtable.go
package main
...
func main() {
	...
	args := flag.Args()
	if len(args) != 3 {
		fmt.Fprintln(os.Stderr, "usage: gencasing -version <X.Y.Z> <UnicodeData.txt> "+
			"<SpecialCasing.txt> <CaseFolding.txt>")
		os.Exit(2)
	}
	...
	foldCommon, foldFull, foldOther := readFolding(args[2])

	var b strings.Builder
	fmt.Fprintf(&b, `// Code generated by cmd/gencasing from Unicode's UnicodeData.txt,
// SpecialCasing.txt and CaseFolding.txt. DO NOT EDIT.
...
//
// And the full case folding, CaseFolding.txt's status C and F: %d characters
// fold to one other and %d to more than one. The %d entries of status S and T
// are left out; see cmd/gencasing. casefold.go reads them.
...
`, *version, len(simple[0]), len(simple[1]), len(simple[2]),
		...
		len(foldCommon), len(foldFull), foldOther)
	...
	b.WriteString("\n// foldCommon: CaseFolding.txt's status C, what these characters fold " +
		"to,\n// one for one.\nvar foldCommon = [...]simpleCase{\n")
	for _, p := range foldCommon {
		fmt.Fprintf(&b, "\t{%#04X, %#04X},\n", p.r, p.to)
	}
	b.WriteString("}\n")
	b.WriteString("\n// foldFull: CaseFolding.txt's status F, what these characters fold to " +
		"when\n// it is more than one.\nvar foldFull = [...]fullCase{\n")
...
// readFolding reads CaseFolding.txt, whose lines are
//
//	<code>; <status>; <mapping>; # <name>
//
// and returns its status C and F entries, sorted, and how many of the others
// it left out. It fails on anything that would make the two into something
// other than one folding: a C that is not one character, an F that is not
// more than one, a character with both, or a status it does not know.
func readFolding(path string) (common []pair, full []entry, other int) {
	f, err := os.Open(path)
	if err != nil {
		fail(err.Error())
	}
	...
	for line := 1; sc.Scan(); line++ {
		...
		if len(fields) != 4 || strings.TrimSpace(fields[3]) != "" {
			fail(fmt.Sprintf("CaseFolding.txt line %d is not <code>; <status>; <mapping>;", line))
		}
		r := parseRune(fields[0], line)
		status := strings.TrimSpace(fields[1])
		mapped := parseRunes(fields[2], line)
		switch status {
		case "C", "F":
			if was, dup := seen[r]; dup {
				fail(fmt.Sprintf("CaseFolding.txt line %d gives U+%04X status %s, and it "+
					"already has status %s", line, r, status, was))
			}
			seen[r] = status
		case "S", "T":
			other++
			continue
		default:
			fail(fmt.Sprintf("CaseFolding.txt line %d has status %q", line, status))
		}
		n := utf8.RuneCountInString(mapped)
		switch {
		case status == "C" && n == 1:
			common = append(common, pair{r, []rune(mapped)[0]})
		case status == "F" && n > 1:
			full = append(full, entry{r, mapped})
		default:
			fail(fmt.Sprintf("CaseFolding.txt line %d folds U+%04X with status %s to %d "+
				"characters", line, r, status, n))
		}
	}
	if err := sc.Err(); err != nil {
		fail(err.Error())
	}
	if len(common) == 0 || len(full) == 0 {
		fail("CaseFolding.txt has no folding of one kind; it is not the file this reads")
	}
	sort.Slice(common, func(a, b int) bool { return common[a].r < common[b].r })
	sort.Slice(full, func(a, b int) bool { return full[a].r < full[b].r })
	return common, full, other
}
