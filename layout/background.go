...
func (l *layouter) canvasBackgroundSource(root *Fragment) *Fragment {
	...
	//
	// The element's box among the wrapper's children is the table itself, and
	// not whichever child comes first with an element behind it. A caption
	// generated by "html::before" comes first and carries the root element too,
	// as every pseudo-element carries its originating one — so the search took
	// the caption for the root, made its background the page's, and left the
	// caption itself unpainted. root-box-002 is that document, whose caption
	// has a white background of its own and a root with none.
	if root.Box != nil && root.Box.TableWrapper {
		for _, c := range root.Children {
			if c.Box != nil && c.Box.Element != nil && c.Box.Inner == InnerTable {
				root = c
				break
			}
		}
