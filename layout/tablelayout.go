...
func (l *layouter) buildTableGrid(table *Box) *tableGrid {
	...
	for _, c := range table.Children {
		...
		case isRowGroup(c):
			...
			switch ascii.Lower(ascii.TrimCSSSpace(c.Style.Get("display"))) {
...
func (l *layouter) spacingOf(table *Box) tableSpacing {
	...
	raw := ascii.TrimCSSSpace(table.Style.Get("border-spacing"))
	if raw == "" {
		return tableSpacing{}
	}
	fields := ascii.CSSFields(raw)
...
func tableLayoutIsFixed(table *Box) bool {
	return ascii.EqualFold(ascii.TrimCSSSpace(table.Style.Get("table-layout")), "fixed")
}
...
func (l *layouter) layoutCells(table *Box, g *tableGrid, cols []style.Unit,
	...
	for _, c := range g.cells {
		...
		out = append(out, placedCell{
			...
			align:   ascii.Lower(ascii.TrimCSSSpace(c.box.Style.Get("vertical-align"))),
...
// between cells with the *table's* background rather than theirs — so a column's
// aqua is interrupted by a stripe of whatever is behind the table at every row
// gap, and a row's is interrupted at every column gap. In the collapsing model
...
func (l *layouter) assembleRows(parent *Fragment, g *tableGrid, placed []placedCell,
	...
	for r, info := range g.rows {
		...
		if info.group >= 0 && groupFrags[info.group] != nil {
			...
			// A row inside a group is placed against the group, and its bands
			// are in the grid's coordinates like every other rectangle here, so
			// they move with it.
			for k := range frag.bgBands {
				frag.bgBands[k].X = frag.bgBands[k].X.Sub(group.BorderRect.X)
				frag.bgBands[k].Y = frag.bgBands[k].Y.Sub(group.BorderRect.Y)
			}
		} else {
			parent.Children = append(parent.Children, frag)
		}
	...
	for _, p := range placed {
		...
		if s.collapsed == nil && cellIsEmpty(p.frag) && ascii.EqualFold(
			ascii.TrimCSSSpace(c.box.Style.Get("empty-cells")), "hide") {
