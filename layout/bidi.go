...
func bidiModeOf(b *Box) bidiMode {
	switch strings.ToLower(strings.TrimSpace(b.Style.Get("unicode-bidi"))) {
	case "embed":
		return bidiEmbed
	case "isolate":
		return bidiIsolate
...
func isRTL(b *Box) bool {
	return strings.EqualFold(strings.TrimSpace(b.Style.Get("direction")), "rtl")
}
...
func (l *layouter) splitByLevel(item inlineItem, para *bidi.Paragraph) []inlineItem {
	...
	for i := 0; i < len(runes); {
		...
		piece.Width = l.br.MeasureSpacedInContext(item.Face, piece.Text, item.Size,
			item.Spacing, shaping{ContextKerns: true, Upright: item.Upright, Off: item.Off})
		if i > 0 {
