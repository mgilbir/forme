...
//
// The line is 17.84375: the default face here is Times, which states no line
...
// is 1142.784, so 1142 units. See lineMetrics for why the glyph box and not the
// AFM's ascender and descender.
...
func TestTableCellHeightIsAMinimum(t *testing.T) {
	...
	cell := find(t, root, "c")
	// Three line boxes, each quantised to the layout unit before they are added
	// up — which is not the same number as quantising their sum.
	if want := mustPx(17.84375).Mul(3); cell.BorderRect.H != want {
