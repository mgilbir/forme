...
func TestOtherSpaceSeparatorsAreWhiteSpaceForPhaseII(t *testing.T) {
	for _, r := range separators {
		...
		pieces, _ := splitAtBreaks("ab"+string(r)+"cd", whiteSpaceOf("preserve"), wordBreak{}, lineBreak{}, hyphens{}, writingSystemOther)
		if len(pieces) != 3 {
...
func TestOghamSpaceMarkIsRemovedAtALineEnd(t *testing.T) {
	...
	pieces, _ := splitAtBreaks("a\u1680\u1680b", whiteSpaceOf("collapse"), wordBreak{}, lineBreak{}, hyphens{}, writingSystemOther)
	if len(pieces) != 4 {
