...
func borderBoxSizing(b *Box) bool {
	return ascii.EqualFold(ascii.TrimCSSSpace(b.Style.Get("box-sizing")), "border-box")
}
...
func sizingKeyword(raw string) string {
	name := ascii.Lower(ascii.TrimCSSSpace(raw))
	if i := strings.IndexByte(name, '('); i > 0 {
		name = ascii.TrimCSSSpace(name[:i])
	}
...
func (l *layouter) checkIntrinsicSizing(b *Box) {
	for _, prop := range sizingProperties {
		raw := ascii.TrimCSSSpace(b.Style.Get(prop))
		if raw == "" {
			continue
		}
